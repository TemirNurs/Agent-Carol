...
- Return ONLY valid JSON, no markdown or explanation"""
    ...
    def _togal_measurements_to_items(self, togal_data: dict) -> list:
        ...
        items = []
        measurements = togal_data.get("measurements", {})

        for sheet_name, sheet_data in measurements.items():
            rooms = sheet_data.get("rooms", [])
            regions = sheet_data.get("regions", [])
            totals = sheet_data.get("totals", {})

            total_room_sf = totals.get("total_room_sf", 0)
            total_perim_lf = totals.get("total_room_perim_lf", 0)

            if total_room_sf > 0:
                # Classify based on sheet name patterns
                sheet_upper = sheet_name.upper()
                if "RCP" in sheet_upper or "301" in sheet_upper or "302" in sheet_upper:
                    items.append({
                        "area": f"Ceiling - {sheet_name}",
                        "task_code": "ceiling_drywall_spray",
                        "quantity": round(total_room_sf, 0),
                        "unit": "SF",
                        "method": "spray",
                        "coats": "1P+2F",
                        "notes": f"Togal AI: {len(rooms)} rooms detected",
                    })
                elif "FINISH" in sheet_upper or "901" in sheet_upper or "902" in sheet_upper:
                    # Finish plans — wall areas from perimeter
                    if total_perim_lf > 0:
                        wall_sf = round(total_perim_lf * 10, 0)  # 10ft default height
                        items.append({
                            "area": f"Walls - {sheet_name}",
                            "task_code": "walls_new_drywall_prime_2coat",
                            "quantity": wall_sf,
                            "unit": "SF",
                            "method": "B&R",
                            "coats": "1P+2F",
                            "notes": f"Togal AI: {total_perim_lf:.0f} LF perim x 10ft",
                        })
                else:
                    # Floor plans — use room areas for wall calculation
                    if total_perim_lf > 0:
                        wall_sf = round(total_perim_lf * 10, 0)
                        items.append({
                            "area": f"Walls - {sheet_name}",
                            "task_code": "walls_new_drywall_prime_2coat",
                            "quantity": wall_sf,
