...
def find_threads_for_bid(M, project_name: str, gc_name: str, log_fn) -> list:
    ...
    threads = []
    threads.extend(gmail_search(M, "INBOX", query))
    threads.extend(gmail_search(M, '"[Gmail]/Sent Mail"', query))
...
def build_timeline(threads: list, submitted_date: str) -> dict:
    ...
    # Classify each thread by direction
    inbound = [t for t in threads if "[Gmail]/Sent Mail" not in t["folder"]]
    outbound = [t for t in threads if "[Gmail]/Sent Mail" in t["folder"]]

    # Heuristics: find ITB invitation, proposal sent, follow-ups, GC replies
    itb = None
    for t in inbound:
        subj = t["subject"].lower()
        if any(k in subj for k in ("invit", "itb", "bid", "request", "rfq", "rfp")):
            itb = t; break
    if not itb and inbound:
        itb = inbound[0]

    proposal = None
    for t in outbound:
        subj = t["subject"].lower()
        if any(k in subj for k in ("proposal", "bid", "quote")) and "follow" not in subj and "fu" not in subj:
            proposal = t; break

    followups = [t for t in outbound if any(k in t["subject"].lower()
                  for k in ("follow", "fu", "checking in", "status"))]
    gc_replies = inbound[1:] if itb else inbound  # everything after the initial ITB
...
def extract_loss_hints(threads: list) -> list[str]:
    ...
    hints = []
    for t in threads:
        if "[Gmail]/Sent Mail" in t["folder"]:
            continue  # only look at inbound
        body = t["snippet"]
...
def fmt_email(t: dict) -> str:
    """One-liner for a thread."""
    d = t["date"].strftime("%Y-%m-%d") if t["date"] else "?"
    direction = "→" if "[Gmail]/Sent Mail" in t["folder"] else "←"
    other = t["to"] if direction == "→" else t["from"]
