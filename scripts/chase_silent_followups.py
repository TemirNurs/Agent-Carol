...
def has_replied_since(project_name, contact_email, since_date, bid_id=None,
    ...
    try:
        ...
        # day or two before our previous fire (an earlier real incident), but NOT so
        ...
        # (a real ~$300K bid where a 7-day window caught a GC pre-submission
        # email and wrongly marked it "replied").
...
def main():
    # GLOBAL CHASE-BATCH LOCK — refuses to run if ANY other chase batch
    # is currently alive (chase_silent_followups, _chase_today,
    # chase_consolidated, send_followups_throttled, _send_today_chases_*).
    # The user explicitly forbade running multiple chase pipelines (2026-05-25).
    ...
    try:
        ...
        _atx.register(_global_release)
    except ImportError as _e:
        print(f"WARN: global chase lock unavailable ({_e}) — refusing to run "
              "rather than risk a double chase", file=sys.stderr)
        sys.exit(3)

    # Per-script lock (legacy belt-and-suspenders for THIS script's reentrancy)
    if not acquire_lock():
        try:
    ...
    for key, info in list(state.items()):
        ...
        if args.apply:
            # Pass --internal-id so send_email tags subject with [ID:xxxxxxxx],
            # which the reply processor uses for shift-immune lookup.
            internal_id = (d.get("Internal ID") or "").strip()
            cmd = [sys.executable, str(SEND_EMAIL),
                   ...
                   "--no-signature"]
            if internal_id:
                cmd.extend(["--internal-id", internal_id])
...
if __name__ == "__main__":
    # RETIRED 2026-06-16 (god-level rebuild): collapsed to ONE pipeline. The
    # escalating "hit them harder" cadence is what the full-history audit found
