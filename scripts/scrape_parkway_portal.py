...
Parkway is one of CCF's top GCs by volume. They post their bidding projects
on a private portal at parkwayconstructionplans.com.
...
# CCF HQ origin (Monroe, NC) — used for distance math.
...
def distance_from_ccf(city: str, state: str) -> float | None:
    ...
    Resolution order (most → least accurate):
      1. city_geocode_cache.json — real geocoded coords ('city|STATE').
      2. static CITY_COORDS city-level entry.
      3. state-center fallback — APPROXIMATE; over-estimates border cities.
    _Why this order: 2026-06-04 the state-center fallback placed Harrogate, TN
    (a real Parkway project) at 342mi when it's actually ~206mi from Monroe, so
    it was wrongly auto-skipped as out-of-range. Parkway is our #1 GC — silently
    dropping an in-range project on a bad estimate is exactly what to avoid._
    """
    if not state:
        return None
    c = (city or "").strip()
    st = state.strip()
    # 1. Accurate geocode cache (real coords from the geocoder)
    if c:
    ...
    key = f"{c.lower()}, {st.lower()}" if c else st.lower()
