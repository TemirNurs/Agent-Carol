...
Parses invitation emails from GC private portals (e.g. SmartBidNet), iSqFt
transmittals (isqftmail.com), direct GC emails, and forwarded invitations from
team gmail accounts. Extracts project name, GC, due date, city/state, SF, and
source URL. Writes unique entries to active_bids.json with `source: "email"`.
...
def fetch_invitations(days=7):
    ...
    for q in queries:
        ...
        for mid in ids[0].split():
            ...
            # Dual-company inbox guard: this Gmail also receives other-company
            # invites (e.g. telecom). Only ingest mail addressed to OUR domain
            # — otherwise foreign ITBs enter the bid list with fabricated GC
            # names (the WSSU/Elford contamination incident).
            from _lib import company as _co
            rcpt = " ".join(filter(None, (msg.get(h, "") for h in
                   ("To", "Cc", "Delivered-To", "X-Original-To")))).lower()
            if rcpt and _co.company_domain().lower() not in rcpt:
                continue
            subj = ""
...
def extract_location(body):
    ...
    # NOTE: hyphen must be in the city class — without it "Winston-Salem, NC"
    # fails at "Winston-" and re-matches as city="Salem" (real CRM corruption).
...
def make_bid_record(email_data):
    ...
    # Trade gate FIRST (config-driven): drop invites that are clearly another
    # trade and name none of OUR scope — before any other parsing, so foreign
    # ITBs reject for the right reason even when the project name is
    # unparseable. "All Trades"/"Main Trades" ITBs pass.
    from _lib import company as _co
    blob = f"{subj}\n{body[:3000]}".lower()
    if (any(t in blob for t in _co.trade_reject_keywords())
            and not any(p in blob for p in _co.trade_keywords())):
        return None, "non_ccf_trade"
    ...
    # Junk guard (6/29): never cache an internal teammate as a "GC" (means GC
    # extraction failed and fell back to the sender), and never cache a row whose
    # project name is just the city — both are parse failures that polluted
    # active_bids.json (e.g. "Monroe, NC" / gc="<a teammate name>"). Real forwards
    # where extract_gc found the true GC (e.g. "Parkway Construction") still pass.
    # Team name tokens come from TEAM_NAME_TOKENS in .env (kept out of source).
    _INTERNAL_GC = {t.strip().lower() for t in os.environ.get("TEAM_NAME_TOKENS", "").split(",") if t.strip()}
    if (gc or "").strip().lower() in _INTERNAL_GC:
        return None, "internal_sender_gc"
    _c, _s = str(city or "").strip(), str(state or "").strip()
    if name.strip().lower() in (_c.lower(), f"{_c}, {_s}".lower(), f"{_c},{_s}".lower()) and _c:
        return None, "city_as_project_name"

    return {
        "project_name": name,
        "gc": gc or "Unknown",
        "trade": _co.default_trade(),
        "due_date": due or "",
        "city": city or "",
        "state": state or "",
        "source": "email",
        "source_detail": frm[:60],
        # sha1, not builtin hash(): hash() is salted per-process, so the same
        # project got a NEW id every ingest run (duplicate-row fuel downstream).
        "opportunity_id": "em-" + hashlib.sha1(
            (normalize(name) + normalize(gc or "")).encode()).hexdigest()[:10],
        "portal_url": url or "",
        "sf": sf,
...
def dedupe(new, existing):
    ...
    for b in new:
        ...
        if not k:
            continue
        # Containment match, not equality: "campuspoliceimprovements" and
        # "campuspoliceimprovementswssu" are the same project (different
        # GC subject spellings created duplicate entries → duplicate CRM rows).
        if any(k.startswith(e) or e.startswith(k) for e in keys):
            continue
        keys.add(k)
        added.append(b)
    return added
...
def main():
    ...
    if added:
        ...
        from _lib.state import save_json as _state_save
        _state_save(BIDS_FILE, merged)   # atomic + locked + keeps .bak
