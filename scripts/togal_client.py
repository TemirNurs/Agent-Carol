...
TOGAL_API_BASE = "https://app.togal.ai/api"
...
class TogalClient:
    ...
    def list_projects(self) -> list:
        """GET /v1/project — list all projects."""
        resp = self._get("/v1/project")
    ...
    def create_set(self, project_id: str, name: str = "Bid Documents") -> dict:
        """POST /v1/set — create a set within a project."""
        return self._post("/v1/set", json={
            "name": name,
            "project_id": project_id,
        })

    def list_sets(self, project_id: str) -> list:
        """GET /v1/set — list sets for a project."""
        resp = self._get("/v1/set", params={"$where": json.dumps({"project_id": project_id})})
        if isinstance(resp, dict) and resp.get("rows"):
            return resp["rows"]
        return []
    ...
    def list_pages(self, set_id: str) -> list:
        """GET /v1/page — list pages in a set."""
        resp = self._get("/v1/page", params={
