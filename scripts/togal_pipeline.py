...
class TogalPipeline:
    ...
    def _plan_score(pdf: Path) -> float:
        ...
        # Strong positive signals (architectural plan sets)
        plan_kw = [
            "prototype", "architectural", "plan set", "a-set", "a-plans",
            "a1", "a101", "a201", "a301", "floor plan", "permit set",
            "construction set", "cd set", "dd set", "bid set", "drawing",
            "drawings", "plans", "sheets",
        ]
        for kw in plan_kw:
            if kw in name:
                score += 20

        # Strong negative signals (not a plan set)
        reject_kw = [
            ...
            "rfi", "meeting minutes", "schedule", "contract", "insurance",
            "scope letter", "invitation to bid", "itb",
        ]
        for kw in reject_kw:
            if kw in name:
                score -= 100

        # Site plans are OK but lower priority than architectural prototype
        if "site plan" in name or "civil" in name:
            score -= 10

        # Size tiebreaker (bigger plan sets usually have more sheets)
        score += size_mb
    ...
    def upload_plans(self, pdf_path: str) -> dict:
        ...
        # Create set
        togal_set = client.create_set(project["id"], "Bid Documents")
        if togal_set.get("error"):
            return {"error": f"Set creation failed: {togal_set['error']}"}
