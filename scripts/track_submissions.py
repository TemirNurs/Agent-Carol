...
def extract_bid_total(body: str):
    ...
    Proposals format the total like:
      'TOTAL BID PRICE ................................... $XX,XXX'
      'TOTAL COMBINED CCF SCOPE $XX,XXX'
      'GRAND TOTAL: $X,XXX,XXX.00'
      'TOTAL PROPOSAL — $XX,XXX'
    The OLD code grabbed the first $-number anywhere (usually a line item),
    which is why a bid once logged a line-item figure instead of the grand
    total. Strategy:
    ...
    if not body:
        return None
    body = body.replace(" ", " ")
    # 0. HIGHEST priority: an explicit "base bid ... $X" phrase. Emails often
    #    list optional add-scopes LARGER than the base bid (one real case: a
    #    base bid with a larger tile option — max-of-body logged the option as
    #    our bid). The base-bid figure IS the bid; return it directly.
    # [^$] not [^$\n]: proposal emails line-wrap mid-sentence ("...is\n$XX,XXX")
    # and the newline broke this rule, falling back to max-of-body (the larger
    # option). Caught by self_audit.py on its first run.
    # Strict number shape (1-3 digits then ,ddd groups) — naive [\d,]+ caught
    # the trailing comma in "is $XX,XXX, based on" → CRM got "$XX,XXX,".
    m = re.search(r"base\s+bid[^$]{0,120}?\$\s*(\d{1,3}(?:,\d{3})*(?:\.\d{2})?)",
                  body, re.I | re.S)
    if m:
        try:
            ...
            pass
    # Specific labels FIRST; bare "TOTAL" only as fallback — an "OPTION TOTAL
    # $XX,XXX" must never beat "TOTAL BID PRICE $XX,XXX". Amounts preceded by
    # option/alternate/allowance wording are rejected outright.
    SPECIFIC_LABEL = (r"(?:GRAND\s+TOTAL|TOTAL\s+BID\s+PRICE|TOTAL\s+COMBINED"
                      r"[\w\s]*|TOTAL\s+PROPOSAL|PROJECT\s+TOTAL|TOTAL\s+CCF"
                      r"[\w\s]*|BID\s+PRICE|PROPOSAL\s+TOTAL|TOTAL\s+PRICE"
                      r"|CONTRACT\s+(?:SUM|PRICE))")
    OPTIONISH = re.compile(r"(?:option|alternate|add[\s\-]?alt|allowance|"
                           r"separately\s+priced|upgrade)", re.I)

    def _label_candidates(label_re):
        out = []
        for m in re.finditer(label_re + r"[\s\.\:\-–—=>]{0,60}?\$\s*(\d{1,3}(?:,\d{3})*(?:\.\d{2})?)",
                             body, re.I | re.S):
            # Option-wording check on the SAME LINE only — a cross-line window
            # rejects the legit "TOTAL BID PRICE" when the line above is an option.
            _pre = body[max(0, m.start() - 80):m.start()].split("\n")[-1]
            if OPTIONISH.search(_pre):
                continue
            try:
                val = int(m.group(1).replace(",", "").split(".")[0])
            except ValueError:
    ...
    candidates = _label_candidates(SPECIFIC_LABEL)
    if not candidates:
        candidates = _label_candidates(r"(?:TOTAL)")
    if candidates:
...
def main():
    ...
    # Load existing overrides — locked/atomic layer: torn files fall back to
    # .bak instead of crashing the daemon job.
    from _lib.state import load_json as _state_load
    data = _state_load(STATUS_FILE, default={"overrides": {}, "history": []})
    ...
    # created duplicate rows (chase emails to a GC's bids@ inbox got logged
    # as fresh proposals). An ORIGINAL proposal subject says
    ...
    for p in proposals:
        ...
        if is_orphan:
            ...
            # includes bare "bid" + "ccf" so a subject like "CCF Bid — <project>"
            # counts (the old narrow list silently dropped a large proposal).
            ...
            _internal_markers = (
                ("carolinacommercial", "mailer-daemon", "noreply", "no-reply")
                + tuple(d.strip() for d in os.environ.get(
                    "CCF_OWN_DOMAINS", "carolinacommercial").split(",") if d.strip())
                + tuple(a.strip() for a in os.environ.get(
                    "OWNER_ALIAS_EMAILS", "").split(",") if a.strip()))
            ext_recipient = recipient_email and not any(
                x in recipient_email.lower() for x in _internal_markers)
    ...
    if updated:
        ...
        _state_save(STATUS_FILE, data)   # atomic + locked + keeps .bak
