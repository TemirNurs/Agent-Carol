...
def get_body_text(msg) -> str:
    ...
    # Prefer text/plain when present; fall back to HTML-stripped when plain
    # is empty (Outlook web frequently sends HTML-only — a real GC reply was
    # HTML-only and we missed the intel).
    # Operator-precedence fix (5/27): parens around the ternary so plain.strip()
    # is the primary, with HTML fallback if plain is empty.
    if text_plain.strip():
        body = text_plain
    elif text_html:
        body = _html_to_text_local(text_html)
    else:
        body = ""
    body = re.sub(r"\r\n?", "\n", body)
...
def resolve_internal_id(sender: str, subject: str, body: str, cache: dict) -> str | None:
    ...
    Strategy (most → least reliable):
      0. Extract [ID:xxxxxxxx] tag from subject (8-char Internal-ID prefix that
         send_email.py --internal-id appends). This is FULLY shift-immune and
         doesn't depend on sender/project at all.
    ...
    # 0. The stable resolver — [ID:xxxxxxxx] tag in subject. Try first, but VERIFY.
    # The tag is the first 8 hex chars of the Internal ID UUID; we match by prefix.
    # CAVEAT: a handful of HISTORICAL sent emails were stamped with the WRONG Internal
    # ID (an old ID reassignment left ~10 sent subjects pointing at the wrong CRM row,
    # caught by audit_chase_subject_ids.py). Blindly trusting such a tag would misroute
    # the reply. So we accept the tag ONLY if the row it resolves to shares a
    # distinctive project token with the subject (or neither side has tokens to
    # compare). On a mismatch we treat the tag as corrupt and fall through to the
    # sender/domain + subject resolver below.
    id_tag = re.search(r"\[ID:([a-f0-9]{6,32})\]", subject or "", re.I)
    if id_tag:
        prefix = id_tag.group(1).lower()
        for row_idx, rec in cache.get("rows_idx", []):
            iid = str(rec.get("Internal ID") or "").strip()
            if iid and iid.replace("-", "").lower().startswith(prefix):
                ptoks = set(proj_tokens(rec.get("Project Name", "")))
                stoks = set(proj_tokens(subject))
                if not ptoks or not stoks or (ptoks & stoks):
                    return iid          # tag agrees with subject → trust it
                break                   # tag → an UNRELATED project: corrupt tag,
                                        # stop trusting it and fall through
        # Tag present but unmatched/corrupt — fall through to fuzzier methods rather
        # than refusing (stale ID after a deleted bid, a typo, or a known mis-tag).
        ...
        if rec:
            ...
            for tok in proj_tokens(rec.get("Project Name", "")):
                if tok in subj_l and iid:
                    return iid
    # No safe match — DO NOT guess. The old "pick a row" fallback glued the
    # Aylett LOST onto Montpelier (it also picked the LOWEST row while the
    # comment claimed newest). Ambiguous replies go to manual review instead.
    try:
        print(f"  [resolve] AMBIGUOUS reply (sender matches {len(pool)} bids, "
              f"no distinctive subject token) — skipped for manual review")
    except Exception:
        pass
    return None
...
def classify_with_regex_fallback(reply: dict) -> dict | None:
    """Catch obvious LOST / WON / REDIRECT / PRICING patterns BEFORE asking
    Gemini. Returns a classification dict if confidence ≥ 0.85, else None
    (fall through to Gemini). The 5/26 incident showed Gemini classified
    short blunt replies like 'Example GC was not awarded the bundle'
    as UNCLEAR conf=0.0 — that lost real intel. Patterns here cover the
    most common GC reply phrasings."""
    text = f"{reply.get('subject','')} {reply.get('body','')}".lower()
    raw = f"{reply.get('subject','')} {reply.get('body','')}"

    # ── REDIRECT (check FIRST — implies project still alive) ─────────────
    # Common phrasings:
    #   "Please contact: X" / "Please contact X"
    #   "Reach out to the PM, X"
    #   "Project Manager is X" / "PM: X"
    #   "Bid info turned over to project manager X"
    #   "Awarded to <Our GC>, contact PM X" (sub-award still open)
    # MOVED ABOVE LOST per 5/27 incident: a GC's "PM has not awarded any
    # contracts" was wrongly classified as LOST because his reply also said
    # "reach out to PM <contact name> at <email>..."
    redirect_m = re.search(
        r"(?:please\s+contact|"
        r"contact(?:\s+the|\s+our)?\s+(?:PM|project\s+manager)|"
        r"reach\s+out\s+to(?:\s+the|\s+our)?(?:\s+project\s+manager|\s+PM)?|"
        r"new\s+contact\s+(?:is|will\s+be)|"
        r"turned\s+over\s+to\s+(?:the\s+)?project\s+manager|"
        r"project\s+manager(?:\s+is|\s*[:,])?)"
        r"[,:\s]+"
        r"([A-Z][\w\.\- ]+?)?"
        r"\s*(?:at|@|\s|,|\.)"
        r"\s*([\w\.\-\+]+@[\w\.\-]+)?",
    ...
    if redirect_m:
        name = (redirect_m.group(1) or "").strip(" ,:")
        email = (redirect_m.group(2) or "").strip(" ,:").lower()
        # 6/16 fix: if the phrase didn't capture the email (e.g. it was in
        # PARENS — "<PM name> (<email>)" — the old regex's
        # `at|@` separator missed it, so Salisbury VAMC kept chasing the old PM),
        # pull the first address appearing right after the redirect phrase.
        if not email:
            tail = raw[redirect_m.start():redirect_m.start() + 200]
            em = re.search(r"[\w\.\-\+]+@[\w\.\-]+\.\w{2,}", tail)
            if em:
                email = em.group(0).strip(" ,:().<>").lower()
        if name or email:
            target = name or email
            return {"category": "REDIRECT", "confidence": 0.90,
                    "summary": f"Redirect contact to {target}",
                    "redirect_to": name, "redirect_email": email, "_source": "regex"}

    # ── LOST — explicit phrasings ────────────────────────────────────────
    # Patterns require LOST to be subject-bound to US (we / you / your bid)
    # — generic "X has not awarded contracts yet" is NOT lost, it's pending.
    lost_phrase = re.search(
        ...
        r"(?:submitted\s+after|unable\s+to\s+use\s+your\s+quote|past\s+the\s+(?:hard\s+)?deadline|after\s+the\s+(?:hard\s+)?deadline)|"
        r"(?:we\s+(?:are|were)\s+not\s+bidding|not\s+bidding\s+(?:this|that|the)|isn'?t\s+bidding|"
        r"won'?t\s+be\s+bidding|won'?t\s+pursue)|"
        r"(?:over\s+\$?[\d,]+\s+above|(?:thousand|\$\d+k)\s+above\s+our))",
        text)
    # Pricing intel — common phrasings: "low (number|bid|price) was $X"
    low_m = re.search(r"(?:low|lowest|cheapest)\s+(?:number|bid|price|guy)?"
                      r"\s*(?:was|came\s+in\s+at|is)?\s*[~]?\s*\$?\s*"
                      r"(\d{1,3}(?:[,.]?\d{3})*[kK]?)", text)
    if lost_phrase or (low_m and re.search(r"\byou\s+(?:were|are)\s+(?:high|over)|\bhigh\.\s*my\b", text)):
        loss_reason = "GC says not awarded"
        if low_m:
            low_raw = low_m.group(1).lower().replace(",", "").replace(".", "")
            # Handle "18k" shorthand
            if low_raw.endswith("k"):
                low_val = int(low_raw[:-1]) * 1000
            else:
                low_val = int(low_raw)
            loss_reason = f"Pricing — competitor low was ${low_val:,}; we were high"
        return {"category": "LOST", "confidence": 0.95, "loss_reason": loss_reason,
                "summary": loss_reason, "_source": "regex"}

    # WON
    if re.search(r"\b(?:awarded\s+to\s+you|you\s+(?:got|won|were\s+awarded)\s+(?:the|this|our)|"
                 r"congratulations.*award)", text):
        return {"category": "WON", "confidence": 0.95, "summary": "Awarded to us", "_source": "regex"}

    # (REDIRECT moved to top of function — sub-award redirect implies project
    # still alive, must be checked before LOST regex which can false-match on
    # "PM has not awarded contracts yet".)

    # AWARDED_TO_GC — the prime GC won, sub-award still open. Treat as
    # STILL_PENDING (the project IS alive; we just need to wait for sub-award
    # decision). Don't close. Common phrasing: "this project has been awarded
    # to <Company>".
    if re.search(r"(?:project|this)\s+(?:has\s+been\s+)?awarded\s+to\s+\w", raw, re.I):
        return {"category": "STILL_PENDING", "confidence": 0.85,
                "summary": "GC won the prime — sub-award pending PM decision",
                "_source": "regex"}

    # OUT_OF_OFFICE
    if re.search(r"(?:out\s+of\s+(?:the\s+)?office|on\s+vacation|on\s+leave|"
                 r"auto[\-\s]?reply|automatic\s+reply)", text):
        return {"category": "OUT_OF_OFFICE", "confidence": 0.95,
                "summary": "Auto-reply / OOO", "_source": "regex"}

    # STILL_PENDING — explicit "no decision yet"
    if re.search(r"(?:no\s+(?:official\s+)?award|no\s+(?:news|decision|update)\s+yet|"
                 r"still\s+(?:pending|under\s+review|in\s+(?:the\s+)?owner'?s?\s+hands)|"
                 r"still\s+working\s+towards|please\s+check\s+back)", text):
        return {"category": "STILL_PENDING", "confidence": 0.90,
                "summary": "Still pending owner decision", "_source": "regex"}

    return None   # No high-confidence regex match — fall through to Gemini


def classify_with_gemini(reply: dict) -> dict:
    # Regex classifier is the PRIMARY path now — deterministic, free, fast,
    # and reliable. The 5/26 incident proved Gemini was unreliable (returned
    # UNCLEAR conf=0.0 on obvious LOST replies). User explicit rule (5/26):
    # "why are you using gemini to update CRM? you are the claude code,
    # can't you do it without stupid gemini?"
    rx = classify_with_regex_fallback(reply)
    if rx and rx.get("confidence", 0) >= 0.85:
        return rx

    # Skip Gemini entirely in the autonomous daemon path. Mark unclear so the
    # user (or Carol-on-Telegram interactively) can review during a session.
    # The CRM row stays untouched; we just log the reply for visibility.
    if os.environ.get("CAROL_ALLOW_GEMINI_CLASSIFY") != "1":
        return {"category": "UNCLEAR", "confidence": 0.0,
                "summary": "regex didn't match a clear pattern — needs manual review",
                "_source": "regex-only"}

    # Manual-only path (set CAROL_ALLOW_GEMINI_CLASSIFY=1 to use Gemini for
    # ambiguous cases). Not used by the daemon.
    try:
        import litellm
    except ImportError:
...
def update_crm(internal_id: str, classification: dict, reply_date: datetime | None,
    ...
    # REDIRECT: auto-switch Contact Email to the new person + move current
    # contact to CC. User rule (5/27): "a GC contact redirected us to a new PM
    # TWICE and we kept chasing the old contact" — the classifier MUST update the contact, not
    # just flag for review.
    if cat == "REDIRECT" and conf >= 0.85:
        new_email = (classification.get("redirect_email") or "").strip().lower()
        new_name = (classification.get("redirect_to") or "").strip()
        cur_email = (rec.get("Contact Email") or "").strip().lower()
        cur_cc = (rec.get("CC Contacts") or "").strip()
        if new_email and "@" in new_email and new_email != cur_email:
            contact_email_col = headers.index("Contact Email") + 1 if "Contact Email" in headers else None
            contact_name_col = headers.index("Contact Name") + 1 if "Contact Name" in headers else None
            cc_contacts_col = headers.index("CC Contacts") + 1 if "CC Contacts" in headers else None
            if contact_email_col:
                cell_updates.append({"range": rowcol_to_a1(row_idx, contact_email_col),
                                      "values": [[new_email]]})
            if contact_name_col and new_name:
                cell_updates.append({"range": rowcol_to_a1(row_idx, contact_name_col),
                                      "values": [[new_name]]})
            if cc_contacts_col:
                # Push the old primary into CC (without duplicating)
                new_cc = cur_cc
                if cur_email and cur_email not in (new_cc or "").lower():
                    new_cc = (cur_cc + ", " + cur_email).strip(", ") if cur_cc else cur_email
                cell_updates.append({"range": rowcol_to_a1(row_idx, cc_contacts_col),
                                      "values": [[new_cc]]})
            changes["redirected_to"] = new_email
            changes["redirected_from"] = cur_email
        pass
