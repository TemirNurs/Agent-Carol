...
Usage from another script:
  ...
  log_activity("📊 CRM", "Updated BID-0057 Status: Awaiting -> Lost (GC reply, bid over winner)")
...
# Generic markers + a per-teammate "<name> asked/said" built from the
# TEAM_FIRST_NAMES env var (names kept out of tracked source).
_GENERIC_LEAK_MARKERS = ("user said", "you asked", "you said", "private message")
_TEAM_FIRST = [n.strip().lower() for n in os.environ.get("TEAM_FIRST_NAMES", "").split(",") if n.strip()]
CHAT_LEAK_MARKERS = tuple(_GENERIC_LEAK_MARKERS) + tuple(
    f"{n} {verb}" for n in _TEAM_FIRST for verb in ("asked", "said")
)
