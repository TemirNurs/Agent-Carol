...
def main():
    ...
    def line(r):
        ...
        # LOCAL star = NC/SC AND within ~2hr drive (≤120mi). If no distance is
        # known, allow the star for NC/SC (benefit of the doubt). A 258mi
        # project is NOT local — starring by state alone was misleading the
        # sweet-spot prioritization.
        d = r["mi"]
        is_local = r["st"] in ("NC", "SC") and (
            not isinstance(d, (int, float)) or d <= 120)
        star = " *" if is_local else ""
        return (f"  • {r['proj']} — {r['gc'] or '(GC ?)'} — "
                f"{r['city']}, {r['st']} {mi}{star}".rstrip())
