...
STORE_PATTERNS = [
    re.compile(r"#\s*(\d{3,4})[\-A-Z]?", re.I),       # "#0946" / "#0946-A"
    re.compile(r"(?:store|#)\s*[#:]?\s*(\d{4})", re.I),  # "Store #0946"
]
...
def extract_store_amounts(body: str, subject: str) -> list[dict]:
    ...
    Looks for patterns like:
      - "Food Lion #0946 ... $25,511"
      - "#0946-A — Randleman, NC ... TOTAL: $25,511.00"
...
def main():
    ...
    for q in [
        ...
        for mid in ids[0].split():
            ...
            if any(x in to_field.lower() for x in ("wilsonsviatlana83", "smayurov@gmail",
                                                    "estimates@carolina", "cs@carolina")):
                # Could still have GC recipient — check carefully
                non_internal = [a for a in re.findall(r"[\w.%+-]+@[\w.-]+\.[A-Za-z]{2,}", to_field)
                                if not any(x in a.lower() for x in ("wilsonsviatlana83", "smayurov@gmail",
                                                                     "@carolinacommercial"))]
                if not non_internal:
                    continue
            body = get_body(msg)
    ...
    for p in all_proposals:
        ...
        for tup in rcpts:
            ...
            if not any(x in addr for x in ("wilsonsviatlana83", "smayurov@gmail",
                                            "@carolinacommercial")):
    ...
    for m in missing:
        ...
        if m["recipient"]:
            ...
            domain_map = {
                ...
                "rcsconstruction.com": "RCS Construction",
                "salcoacontracting.com": "Salcoa Contracting",
                "newcoconstruction.com": "NewCo Construction",
                "wedconstruction.com": "WED Construction",
                "vertexconstruction.com": "Vertex Construction",
            }
