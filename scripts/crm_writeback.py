...
# ---------------------------------------------------------------------------
# Single source of truth for the domain->GC directory + email resolution lives
# in _lib.gc (data loaded at runtime from the gitignored gc_directory.json /
# gc_crm.json). Delegate here so there's one curated roster, not a duplicate.
from _lib import gc as _gc_lib
...
def _gc_info_for_email(email_addr):
    ...
    Delegates to _lib.gc (exact gc_crm.json match -> known-domain map ->
    CamelCase-domain heuristic)."""
    return _gc_lib.info_for_email(email_addr)
...
def _status_rank(s):
    """Pipeline position of a CRM Status string (substring-matched so verbose
    cells like 'Closed - Lost (...)' still rank). Blank=0, unrecognized=99 (LOCKED
    — never clobber a status we don't understand). Used to ADVANCE status forward
    only: a sent bid flips Proposal Drafted->Bid Submitted, but we never downgrade
    or overwrite a decided/terminal/on-hold cell."""
    s = (s or "").strip().lower()
    if not s:
        return 0
    for kw, r in (("won", 7), ("lost", 7), ("withdraw", 7), ("declined", 7),
                  ("no bid", 7), ("duplicate", 99), ("buyout", 6), ("awaiting", 5),
                  ("hold", 5), ("submitted", 4), ("drafted", 3), ("pending review", 3),
                  ("estimating", 2), ("itb", 1)):
        if kw in s:
            return r
    return 99
...
def main():
    ...
    # Load Carol's data — corruption-tolerant reads (fall back to .bak written
    # by _lib.state instead of crashing the CRM job on a torn file).
    from _lib.state import load_json as _state_load
    bids = _state_load(ACTIVE_BIDS, default=[])
    d = _state_load(STATUS_FILE, default={})
    overrides = d.get("overrides", {}) if isinstance(d, dict) else {}
    ...
    for slug, ovr in overrides.items():
        ...
        # Resolve GC name via the single curated directory (_lib.gc); its
        # heuristic fallback already CamelCases the second-level domain.
        gc_name, _, _ = _gc_info_for_email(recip) if recip else ("", "", "")
        bids.append({
            "project_name": proj,
            "gc": gc_name,
            "city": "",
    ...
    for ab, sub, e in matched_pairs:
        ...
        # Status: ADVANCE forward along the pipeline (not just fill-if-blank) so a
        # drafted bid that gets sent auto-flips Proposal Drafted -> Bid Submitted.
        # Only writes when the new status outranks the current one — never
        # downgrades or clobbers a decided/on-hold/unrecognized cell.
        if "Status" in headers:
            raw_s = crm_status_for(ovr)
            s = _canon("Status", raw_s) or raw_s
            cur_s = str(d.get("Status", "")).strip()
            if s and _status_rank(s) > _status_rank(cur_s):
                updates.append((row_idx, "Status", s))
                changes.append(f"status:{cur_s or 'blank'}->{s}")
    ...
    # Same-run dedupe state. `existing` only covers rows ALREADY in the sheet —
    # when active_bids.json holds TWO entries for one project (email-sourced +
    # portal-sourced, as with Campus Police), each pairs with every submission
    # and the guard below never fires, spawning 2 rows per GC in one run.
    _claimed_subs = set()   # (recipient, at) — one CRM row per physical send
    _run_created = set()    # (project_core, gc_domain/name) created this run
    ...
    for ab, sub in unmatched_pairs:
        ...
        # SAME GC. Two inboxes at the same GC domain (e.g. a shared bids@ box
        # and an individual estimator's address) are the same GC, same bid. The
        # old (project + exact-email) match treated them as distinct and spawned
        # duplicate rows every time the daemon re-captured a send to the
        # alternate inbox. Dedupe on (project-core + GC email-domain) instead.
        ...
        # that for dedupe falsely collapses several distinct GCs bidding the
        # same project into "same GC". Compute the GC for THIS specific
        # submission from the recipient first, fall back to ab only if the
        # recipient is unresolvable.
        ...
        if _is_dup:
            ...
            continue
        _sub_key = (_new_recip, (sub or {}).get("at", ""))
        _run_key = (_new_core, _new_dom or _sub_gc.strip().lower()[:12])
        if _sub_key in _claimed_subs or _run_key in _run_created:
            log(f"  SKIP-DUP(run) {ab.get('project_name','')[:40]} -> {_new_recip[:30]} "
                f"(row already queued this run)")
            continue
        _claimed_subs.add(_sub_key)
        _run_created.add(_run_key)
        ...
        row["Bid #"] = bid_id
        # Every new row gets a stable PK at birth — Bid # is display-only and
        # rows without an Internal ID fell out of reply-routing/chase keying.
        if "Internal ID" in row and not row.get("Internal ID"):
            import uuid as _uuid
            row["Internal ID"] = str(_uuid.uuid4())
    ...
    # Bid # is STATIC TEXT (already minted via next_num) — NEVER write the
    # row-position formula here. That formula renumbered the whole CRM when
    # the sheet was sorted (2026-06-09 corruption); the column was frozen to
    # static values and must stay that way. Internal ID is the join key.
    appended = 0
    if new_rows:
        try:
            appended = append_rows("Bid Log", new_rows)
            log(f"[crm-wb] batch append: {appended} rows added (static Bid#)")
        except Exception as e:
