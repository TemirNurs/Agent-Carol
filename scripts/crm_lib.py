...
def _retry(fn, *args, **kwargs):
    """Call fn(*args, **kwargs), retrying on transient gspread errors with
    backoff: 429 (quota) plus 500/502/503 (Google-side hiccups that previously
    killed a job mid-write, leaving half-applied updates)."""
    import gspread
    ...
    for attempt in range(len(_RETRY_DELAYS) + 1):
        try:
            ...
            if status not in _TRANSIENT or attempt == len(_RETRY_DELAYS):
                raise
