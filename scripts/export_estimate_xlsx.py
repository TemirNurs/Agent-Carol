...
def export_estimate(slug: str) -> str:
    ...
    # Row 3: Address (use project name if no address)
    ws.merge_cells(f"A{r}:F{r}")
    c = ws.cell(row=r, column=1, value="N. Myrtle Beach, SC")
    c.font = INFO_FONT
    c.alignment = ALIGN_L
    r += 1

    # Row 4: Bid date / prepared by
    ws.merge_cells(f"A{r}:F{r}")
    c = ws.cell(row=r, column=1, value=f"Bid Date: {bid_date}  |  Prepared by: Carolina Commercial Finishes")
    c.font = INFO_FONT
    c.alignment = ALIGN_L
    r += 1
    ...
    if wallcovering:
        ...
        for n in [
            ...
            "Material pricing at $2.50/SF — verify with manufacturer rep for exact roll pricing and waste factor",
        ]:
