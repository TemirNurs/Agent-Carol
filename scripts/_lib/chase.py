...
# AGENTS_LESSONS.md R4: every chase MUST CC the internal list (accounting + owner + cs@)
...
def has_replied_recently(recip_email: str | None, days: int = 14) -> bool:
    ...
    try:
        ...
        M.login(user, pw)
        # All Mail, not INBOX — gmail rules auto-label GC replies OUT of the
        # inbox, so INBOX misses them and replied GCs keep getting chased.
