...
# Personal email addresses of CCF team members - never count as CRM GC rows.
# (Owner Sergei, GM/Accountant Sviatlana, Estimator Nursultan)
PERSONAL_EMAIL_BLACKLIST = {
    "smayurov@gmail.com",
    "wilsonsviatlana83@gmail.com",
    "nursultan.mayurov@gmail.com",
    "nursm@gmail.com",
    "smayurov78@gmail.com",
}
...
def name_from_email_prefix(email_addr):
    ...
    Examples:
      sam.bass@flblum.com -> 'Sam Bass'
      slocklear@msquareus.com -> 'S. Locklear'
      ross@cinderellapartners.com -> 'Ross'
      bids@drivencontractors.com -> '' (generic)
      tanner.barber@fiicgc.com -> 'Tanner Barber'"""
    ...
    for sep in ("_", "-"):
        if sep in local:
            ...
            if all(p.isalpha() and len(p) >= 2 for p in parts):
                return " ".join(p.title() for p in parts)
    # flastname pattern: single initial + surname (e.g. slocklear, jhibbard)
...
def main():
    ...
    if appended_rows:
        # For appended rows, write the formula so Bid# auto-computes
        BID_FORMULA = '="BID-"&TEXT(ROW()-1,"0000")'
        for r in appended_rows:
            r["Bid #"] = BID_FORMULA
        print(f"[audit] appending {len(appended_rows)} rows at end (with Bid# formula)...")
