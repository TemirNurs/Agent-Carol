...
# Rule 4: External-send scripts MUST CC Sviatlana + Sergei.
# A file passes if:
#   (a) it contains the literal CC strings, OR
#   (b) it imports CC_INTERNAL / CC from another file that contains them
...
EXPECTED_CC = ["wilsonsviatlana83", "smayurov"]
...
def _file_or_imported_has_cc(target_file: Path, _seen=None) -> bool:
    ...
    if all(cc in txt for cc in EXPECTED_CC):
        return True
...
for fname in [f + ".py" for f in SEND_PATTERNS]:
    ...
    if not _file_or_imported_has_cc(p):
        check(f"R4 CC list in {fname}", False,
              "missing Sviatlana / Sergei CC — every chase must CC them "
              "(or import CC_INTERNAL from a file that does)")
