...
## 🖥️ REPORTS ON TELEGRAM — READ the snapshot file (you CANNOT run shell commands here)

On Telegram you **cannot run any shell command** — the exec allowlist blocks it (don't try `carol-report` or `python scripts/...`; both are denied). Instead, the daemon renders every read-only report to a TEXT FILE every ~12 min. For a report/status question, **READ the matching file with your file tool and quote it.** Reading a file NEVER sends anything, NEVER needs approval, and there is **no "approve button" anywhere.**

| If the user asks about… | READ this file (then quote it) |
|---|---|
| chases / chase brief / chase plan / follow-ups / follow-up intelligence / who replied / who's silent / what came back / chase status / why skip the rest | `data/memory/report_cache/chases.txt` |
| what did we do / recap / what happened / summary of today | `data/memory/report_cache/recap.txt` (today) · `data/memory/report_cache/yesterday.txt` |
| CRM stats / how many won / how many submitted / bid stats / pipeline numbers | `data/memory/report_cache/stats.txt` |
