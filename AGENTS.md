...
- `carol-report chases` → today's proposed chase plan + recap (read-only) · `carol-report team` → who has ever messaged Carol

**🚨 The detailed command table further down this file says `python scripts/<x>.py` — that is the WORKSTATION form and is BLOCKED here. When a request matches an intent below, run the `carol-report` command for it. NEVER refuse one of these, never say "run it from a workstation," never try the raw `python scripts/...` (it will be denied). `carol-report <name>` IS how you run these on Telegram:**

| If the user asks about… | Run on Telegram |
|---|---|
| chases / chase brief / chase plan / follow-ups / follow-up intelligence / who replied / who's silent / what came back / chase status / why skip the rest | **`carol-report chases`** |
| what did we do / recap / what happened / summary of today | **`carol-report recap`** (or **`carol-report yesterday`**) |
| CRM stats / how many won / how many submitted / bid stats / pipeline numbers | **`carol-report stats`** |
| how many open / what's open / open projects / what's worth bidding | **`carol-report open`** (count) · **`carol-report open-list`** (the list) |
| today's bids / what's due today / what came in today | **`carol-report today`** |
| who messaged Carol / team transcript | **`carol-report team`** |

That is the ONLY shell command available here. For anything NOT in the table above (live single-bid lookups, drafting, sending, takeoffs, bid_priority, accountant brief) you genuinely cannot run it on Telegram — answer from what you already know or say plainly it needs the full workstation. **NEVER claim there's an "approval card to tap" — there is none.**
